...
class GetMatchCountdown {
  const GetMatchCountdown({CountdownRepository? repository, CalculateCountdown? calculator})
  ...
  Future<Result<Countdown>> call(DateTime kickoffUtc, {DateTime? nowUtc}) {
    ...
    if (repository != null) return repository.getMatchCountdown(kickoffUtc, nowUtc: nowUtc);
    return Future.value(Success(_calculator(targetUtc: kickoffUtc, nowUtc: nowUtc)));
  }
}
