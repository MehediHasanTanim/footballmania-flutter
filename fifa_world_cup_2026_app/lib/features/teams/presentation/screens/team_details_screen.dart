  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (id == null) {
      return const AppScaffold(
        title: 'Team Details',
        body: AppErrorView(message: 'Invalid team id'),
      );
    }
    ...
    return AppScaffold(
      ...
      body: team.when(
        ...
        error: (error, stackTrace) => AppErrorView(
          ...
          onRetry: () =>
              ref.read(teamDetailsProvider(id).notifier).forceRefresh(),
        ),
        data: (value) {
          final teamFixtures = fixtures
              .where((fixture) => fixture.involvesTeam(value.id))
              .toList(growable: false);
          final results = teamFixtures
              .where((fixture) => fixture.isCompleted)
              .toList(growable: false);
          final upcoming = teamFixtures
              .where((fixture) => !fixture.isCompleted)
              .toList(growable: false);
                  ...
                  child: Column(
                    children: [
                      ListTile(
                        ...
                        subtitle: Text(value.group ?? 'TBD'),
                      ),
                      ListTile(
                        leading: const Icon(Icons.person),
                        title: const Text('Coach'),
                        subtitle: Text(value.coach ?? 'Unavailable'),
                      ),
                      ListTile(
                        leading: const Icon(Icons.leaderboard),
                        title: const Text('FIFA ranking'),
                        subtitle: Text(
                          value.fifaRanking?.toString() ?? 'Unavailable',
                        ),
                      ),
                    ],
                  ),
