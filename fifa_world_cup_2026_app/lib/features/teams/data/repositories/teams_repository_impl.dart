...
class TeamsRepositoryImpl implements TeamsRepository {
  const TeamsRepositoryImpl({
    ...
    required TeamsLocalDataSource localDataSource,
    ...
    try {
      if (!forceRefresh && !await _networkInfo.isConnected) {
        return cached.isNotEmpty
            ? Success(
                cached.map((team) => team.toEntity()).toList(growable: false),
              )
            : const FailureResult(
                Failure(message: 'No cached teams available.'),
              );
      }
      final remote = await _remoteDataSource.getTeams();
      await _localDataSource.cacheTeams(remote);
      return Success(
        remote.map((team) => team.toEntity()).toList(growable: false),
      );
  ...
  @override
  Future<Result<Team>> getTeamById(
    int teamId, {
    ...
    try {
      if (!forceRefresh && !await _networkInfo.isConnected) {
        return cached != null
            ...
            : const FailureResult(
                Failure(message: 'No cached team available.'),
              );
      }
