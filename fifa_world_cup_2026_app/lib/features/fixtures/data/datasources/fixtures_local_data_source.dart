...
class FixturesLocalDataSource {
  ...
  Future<void> cacheFixtures(List<FixtureDto> fixtures) async {
    ...
    await _cacheService.putAll<CachedFixture>(
      HiveConstants.fixturesBox,
      {for (final fixture in fixtures) (fixture.id ?? 0).toString(): fixture.toCachedModel()},
    );
  }
  ...
  List<FixtureDto> getFixtures({String? date, int? teamId, String? group, String? stage}) {
    final fixtures = _cacheService
        ...
        .where((fixture) => _matchesFilter(fixture, date: date, teamId: teamId, group: group, stage: stage))
        .toList(growable: false);
    return fixtures;
  }

  FixtureDto? getFixtureById(int fixtureId) {
    final cached = _cacheService.get<CachedFixture>(HiveConstants.fixturesBox, fixtureId.toString());
    if (cached == null || cached.isExpired) return null;
    return FixtureDto.fromCachedModel(cached);
  }

  bool _matchesFilter(FixtureDto fixture, {String? date, int? teamId, String? group, String? stage}) {
    final matchesDate = date == null || fixture.matchDateUtc?.toIso8601String().startsWith(date) == true;
    final matchesTeam = teamId == null || fixture.homeTeamId == teamId || fixture.awayTeamId == teamId;
