...
class GroupsRepositoryImpl implements GroupsRepository {
  ...
  @override
  Future<Result<List<GroupStanding>>> getGroups({
    ...
    try {
      if (!forceRefresh && !await _networkInfo.isConnected) {
        return cached.isNotEmpty
            ? Success(
                cached.map((group) => group.toEntity()).toList(growable: false),
              )
            : const FailureResult(
                Failure(message: 'No cached groups available.'),
              );
      }
      final remote = await _remoteDataSource.getGroups();
      await _localDataSource.cacheGroups(remote);
      return Success(
        remote.map((group) => group.toEntity()).toList(growable: false),
      );
