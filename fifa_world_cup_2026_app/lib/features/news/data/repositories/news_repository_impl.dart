...
class NewsRepositoryImpl implements NewsRepository {
  ...
  Future<Result<List<News>>> getLatestNews({
    ...
    final cached = _localDataSource.getLatestNews(
      page: page,
      pageSize: pageSize,
    );
    try {
      if (!forceRefresh && !await _networkInfo.isConnected) {
        return cached.isNotEmpty
            ? Success(
                cached.map((news) => news.toEntity()).toList(growable: false),
              )
            : const FailureResult(
                Failure(message: 'No cached news available.'),
              );
      }
      final remote = await _remoteDataSource.getLatestNews(
        page: page,
        pageSize: pageSize,
      );
  ...
  Future<Result<List<News>>> searchNews({
    ...
    final cached = _localDataSource.searchNews(
      query: query,
      page: page,
      pageSize: pageSize,
    );
    try {
      ...
      final remote = await _remoteDataSource.searchNews(
        query: query,
        page: page,
        pageSize: pageSize,
      );
